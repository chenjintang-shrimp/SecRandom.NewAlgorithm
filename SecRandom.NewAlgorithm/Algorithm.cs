...
public static class FairDrawWeights
{
    ...
    )
    {
        ...
        for (var index = 0; index < poolSize; index++)
        {
            ...
            var count = drawCountById.GetValueOrDefault(student.Id, 0.0);
            if (count < 0.0)
        ...
        var personalHorizon = settings.PersonalHorizonRounds * poolSize;
        ...
        var dimensionCount = settings.Dimensions.Length;
        var dimensionDebt  = new double[poolSize][];
        for (var index = 0; index < poolSize; index++)
            dimensionDebt[index] = new double[dimensionCount];
        for (var slot = 0; slot < dimensionCount; slot++)
        {
            ...
            for (var index = 0; index < poolSize; index++)
            {
                ...
                var label = labels[dimension.Dimension];
                if (label < 0)
            ...
            for (var index = 0; index < poolSize; index++)
            {
                var label = pool[index].Labels[dimension.Dimension];
                labelShare[label] += share[index];
                labelDrawn[label] += drawCount[index];
            }

            var horizon = dimension.HorizonPerPick * batchSize;
            ...
            for (var index = 0; index < poolSize; index++)
            {
                var value = debt[pool[index].Labels[dimension.Dimension]];
                dimensionDebt[index][slot] =  value;
                weight[index]              *= value;
            }
        }

        var weightSum = weight.Sum();
        var degraded  = weightSum <= 0.0;
        if (degraded)
        {
