...
public static class MetricsCalculator
{
    public static SimMetrics Compute(SimulationResult result)
    {
        ...
        for (var i = 0; i < entries.Count; i++)
        {
            ...
            var lean = e is { Degraded: false, PoolSize: > 1 };
            if (lean)
            {
                leanCount++;
        ...
        var capExact      = minDelta == 0 && maxDelta == 0;
        var totalsOk      = cycleRows.All(row => row == dpc);
        var batchDistinct = CheckBatchDistinct(entries);
        ...
        SimulationResult result, List<StatEntry> stats, int[,] perCycle)
    {
        ...
        if (config.GenderGroupSizes.Length > 1)
        {
            ...
            if (crossTotal > 0)
            {
                var floor      = config.RandomFloor;
                var expectedPa = 0.5 + (1 - floor) * (1 - floor) / (2 * (2 - floor));
        ...
        if (groups > 1)
        {
            ...
            for (var i = 0; i < entries.Count; i++)
            {
                ...
                {
                    var dev = Math.Abs((double)cum[g] / total - expected[g]);
                    if (dev > worst)
                    {
                        worst   = dev;
                        worstG  = g;
    ...
    {
        return total == 0 ? "n/a" : $"{part}/{total} = {(double)part / total:P1}";
    }
}
